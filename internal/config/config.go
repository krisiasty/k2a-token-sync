...
type Cluster struct {
	...
	// SecretName and SecretNamespace locate the ArgoCD cluster Secret this
	// daemon generates and maintains.
	SecretName      string
...
func (c *Config) applyFile(f *file, logger *slog.Logger) error {
	...
	for i, cf := range f.Clusters {
		...
		target := cluster.SecretNamespace + "/" + cluster.SecretName
		if owner, dup := seenSecrets[target]; dup {
			return fmt.Errorf("clusters[%d]: secret %s already generated by cluster %q", i, target, owner)
		}
		seenSecrets[target] = cluster.Name
...
func resolveCluster(cf clusterFile, def *defaults, defTokenTTL, defWarn, defRotate time.Duration) (Cluster, error) {
	...
	out.SecretNamespace = orDefault(cf.SecretNamespace, def.SecretNamespace, "argocd")
