...
`)
	...
	if c.SecretNamespace != "argocd" {
		t.Errorf("SecretNamespace = %q, want argocd", c.SecretNamespace)
	}
...
`)
	...
	for i, c := range cfg.Clusters {
		if c.SecretNamespace != "gitops" {
			t.Errorf("clusters[%d].SecretNamespace = %q, want gitops", i, c.SecretNamespace)
		}
